...
// ─── CONFIG ──────────────────────────────────────────────────
const API_URL =
  "https://ragequitters--mahjong-backend-fastapi-app.modal.run/image/visualise";
const HEALTH_URL =
  "https://ragequitters--mahjong-backend-fastapi-app.modal.run/docs";
// ─────────────────────────────────────────────────────────────
...
export default function PlayScreen() {
  ...
  const handleCamera = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission Required",
        "Camera access is needed to capture your hand.",
      );
      return;
    }
  ...
  const handleGallery = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission Required",
        "Photo library access is needed to pick your hand.",
      );
      return;
    }
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      // Step 3: Cycle status while waiting
      let cycleIndex = 0;
          ...
          <View style={styles.idleContainer}>
            <View style={styles.instructCard}>
              ...
              {[
                {
                  icon: "📸",
                  text: "Lay your tiles face-up on a flat surface",
                },
                { icon: "🔦", text: "Ensure good lighting for best accuracy" },
                { icon: "🤖", text: "AI identifies the optimal discard" },
                {
                  icon: "✨",
                  text: "The suggested tile is highlighted in the result",
                },
              ].map((item, i) => (
            ...
            {elapsedSeconds >= 15 && (
              <View style={styles.warmingCard}>
                ...
                <Text style={styles.warmingText}>
                  The server is starting up — this can take up to 60s on first
                  use. Subsequent requests will be much faster.
                </Text>
              </View>
            )}
          ...
          >
            ...
            <View style={styles.resultHintCard}>
              ...
              <Text style={styles.resultHintText}>
                The highlighted tile is your recommended discard. It maximises
                your winning probability based on Singapore Mahjong rules.
              </Text>
            </View>
